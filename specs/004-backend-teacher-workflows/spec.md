...
- **JSON response impact**: Responses must use only the success, paginated, validation, unauthorized, and tenant-mismatch envelopes or codes declared by OpenAPI for the affected operation. Shared error components such as forbidden, token-rejection, inactive-user/inactive-school, and not-found are not approved for these ten operations unless OpenAPI first documents them on the relevant operation. No backend-local envelope, ad hoc error code, undocumented status code, undocumented field, undocumented filter, or undocumented sort behavior is approved.
...
- **FR-018**: Backend responses MUST match the published OpenAPI success and error envelopes declared for each approved operation. The backend MUST NOT emit forbidden, token-rejection, inactive-user/inactive-school, not-found, or any other undocumented status semantics for these operations unless OpenAPI first documents them on the relevant operation.
...
- **SC-007**: Response-shape verification confirms every operation in this slice returns the documented success or error envelope for successful, validation where applicable, unauthorized, and tenant-mismatch cases exactly as declared by OpenAPI for that operation.
