...
- [ ] T008 Review approved auth operations `login`, `getCurrentUser`, `logout`, and `requestPasswordReset` against `specs/api/openapi.yaml` and record any gaps in `specs/specs/017-auth-session-ui/quickstart.md`
- [ ] T009 Verify whether an approved user-authorized school-selection source exists in `specs/api/openapi.yaml`; if none exists, record the OpenAPI blocking gap in `specs/specs/017-auth-session-ui/quickstart.md` before enabling school-selection UI
...
- [ ] T042 [US2] Block school selection list population and service consumption unless the approved source operation from `specs/specs/017-auth-session-ui/contracts/auth-session-ui-contract.md` is confirmed in `src/services/auth/authService.js`
...
- [ ] T067 [P] Run auth/session quickstart review commands, including timing acceptance checks for 30-second sign-in/bootstrap, 15-second password reset confirmation, and SC-006 session-recovery UAT targets, and record results in `specs/specs/017-auth-session-ui/quickstart.md`
- [ ] T068 [P] Verify no direct Axios usage exists in auth pages, components, layouts, or router guards and record findings in `specs/specs/017-auth-session-ui/quickstart.md`
- [ ] T069 [P] Verify Element Plus component tags use PascalCase in auth Vue files and record findings in `specs/specs/017-auth-session-ui/quickstart.md`
- [ ] T070 [P] Review local or session storage usage for token, password, Authorization, and tenant metadata handling and record findings in `specs/specs/017-auth-session-ui/quickstart.md`
...
- [ ] T072 [P] Run auth/session Vitest tests, verify timing acceptance coverage is included, and record command output in `specs/specs/017-auth-session-ui/quickstart.md`
- [ ] T073 Update frontend roadmap implementation status and any discovered school-selection contract follow-up in `specs/docs/frontend-feature-roadmap.md`
- [ ] T074 If OpenAPI changed for school selection, run Redocly validation and record result in `specs/specs/017-auth-session-ui/quickstart.md`
- [ ] T075 Review feature against `specs/specs/017-auth-session-ui/contracts/auth-session-ui-contract.md` and record deviations or confirmations in `specs/specs/017-auth-session-ui/quickstart.md`
