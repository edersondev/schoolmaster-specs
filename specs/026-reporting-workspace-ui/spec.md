...
7. **Given** a direct route, stale selection, or bookmarked report identifier targets a missing, deleted-without-include, unauthorized, or cross-tenant run, **When** no approved list, request, retry, cancel, delete, or restore response has already returned that run, **Then** the UI shows a safe unavailable or not-found state without calling an undocumented report-run detail operation or revealing protected report details.
...
- **FR-004**: The UI MUST retrieve report catalog options only through the approved catalog operation and MUST show only documented domains, fields, filters, operators, grouping options, sorting options, output formats, and complexity limits; report request catalog use requires report permission, while custom definition catalog use requires report-definition permission.
...
- **FR-039**: The UI MUST hydrate report-run detail only from approved list, request, retry, cancel, delete, restore, or download-adjacent response state and MUST NOT call an undocumented standalone report-run detail operation.
