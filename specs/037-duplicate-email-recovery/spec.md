...
- Q: What should a same-scope recoverable deleted-user collision return? → A: Return `409 recoverable_user_conflict` with the matching user UUID and a `restore` recommendation.
...
- Q: Does `409 recoverable_user_conflict` guarantee that restoration will succeed? → A: No. It confirms only that recovery guidance may be disclosed because the retained user is soft-deleted in the exact active scope and the requester has effective restore authorization; the explicit restore action re-evaluates all current lifecycle constraints and may still fail.
...
1. **Given** a soft-deleted user owns the submitted email in the administrator's exact active scope and the administrator has effective restore authorization, **When** the administrator attempts to create a user with that email, **Then** creation is rejected with `409 recoverable_user_conflict`, no duplicate identity is persisted, and the response contains only the matching user UUID and a `restore` recommendation.
2. **Given** the administrator received an eligible same-scope recovery result, **When** they explicitly restore the retained user, **Then** the original identity and history are preserved and the user becomes available according to existing restore rules.
...
- Direct user creation and invitation-driven user provisioning receive the same identity decision.
...
- **Backend repository impact**: Align every workflow that can create a user with platform-wide, case-insensitive email uniqueness across active and soft-deleted records. Translate duplicate outcomes into documented responses, provide recovery guidance only for authorized same-scope soft-deleted identities, record privacy-preserving duplicate-attempt audits, preserve explicit restore behavior, and add regression and concurrency coverage.
...
- **Specification or contract repository impact**: Clarify global identity ownership, same-scope recovery eligibility, non-disclosing duplicate behavior, and concurrent outcomes. Update the aggregate OpenAPI descriptions and response contracts for affected creation operations before backend behavior changes.
...
- **OpenAPI update required**: Yes. Document the recoverable same-scope soft-deleted-user conflict separately from the generic unavailable-email validation result and define the minimum safe recovery information.
- **Versioned endpoints affected**: `POST /api/v1/users` and `POST /api/v1/account-invitations` when invitation creation would provision a missing platform user. Existing `POST /api/v1/users/{userId}/restore` and `PATCH /api/v1/users/{userId}` remain the recommended follow-up operations.
- **JSON response impact**: Creation operations must preserve existing success envelopes. Duplicate active, inactive, invited, inaccessible, cross-tenant, and unauthorized recovery cases return `422 validation_failed` with `email: "The email is unavailable."`. An eligible same-scope soft-deleted collision returns `409 recoverable_user_conflict` containing only the matching user UUID and a `restore` recommendation.
- **Authentication/authorization impact**: Existing creation permissions remain required. Recovery guidance is available only when the requester is authorized to access and restore the matching user in the already resolved scope. Authorization and tenant resolution occur before any recoverability disclosure.
...
- **Tenant scoping impact**: Email ownership remains platform-wide while user access and recovery remain bound to the preselected platform or school scope. A school-scoped request cannot discover or recover a user from another school or platform scope.
- **Cross-tenant or platform access impact**: Same-scope recovery guidance is never a cross-tenant lookup path. Opposite-scope and cross-tenant collisions remain indistinguishable from other unavailable-email results.
...
- **FR-006**: When the matching identity is soft-deleted, belongs to the requester's exact preselected active scope, and the requester has effective restore authorization, the system MUST reject creation with `409 recoverable_user_conflict`. This result authorizes recovery disclosure only and MUST NOT guarantee that the later restore action will pass its current lifecycle constraints.
...
- **FR-016**: Direct user creation and invitation-driven user provisioning MUST produce consistent identity, privacy, atomicity, and duplicate-email outcomes.
- **FR-017**: The OpenAPI contract MUST document all changed response semantics before backend implementation begins.
- **FR-018**: Tests MUST cover exact duplicates, case variants, surrounding whitespace under documented normalization, every user lifecycle state, same-scope recovery eligibility, unauthorized and cross-tenant non-disclosure, restore follow-up, invitation-driven provisioning, and concurrent creation attempts.
...
- A recoverable conflict may identify a same-scope deleted user only after current tenant and authorization checks prove the requester has effective restore authorization.
...
- Existing email values are not bulk-normalized; they remain case-insensitively reserved and become trimmed and lowercase only when an authorized workflow changes them.
