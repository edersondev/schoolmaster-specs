...
- **FR-009**: Restoration MUST remain an explicit authorized lifecycle action and MUST continue to enforce existing reason, effective-date, tenant, parent-state, dependency, uniqueness, and history rules. School user restoration MUST require both `users.view` and `users.manage` whether invoked through the single-user or bulk lifecycle endpoint.
