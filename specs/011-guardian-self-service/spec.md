...
- **OpenAPI update required**: Yes. The active feature contract and aggregate contract must define operation IDs, paths, request schemas, response schemas, field visibility, error envelopes, authorization rules, tenant behavior, and non-enumerating denial behavior before backend implementation begins. This includes any required school-admin guardian-user-link provisioning or deactivation operations if they are not already published by an existing admin contract.
- **Versioned endpoints affected**: Approved contract surface is limited to guardian self-service operations under `/api/v1/guardian/students`, `/api/v1/guardian/students/{studentProfileId}`, `/api/v1/guardian/students/{studentProfileId}/academics`, and `/api/v1/guardian/students/{studentProfileId}/contacts`, plus school-admin guardian-user-link provisioning under `/api/v1/guardians/{guardianId}/user-links`.
...
- **FR-006a**: The backend MUST provide a documented same-school school-admin path to create and deactivate guardian-user links required for guardian self-service, either by extending the existing guardian administration surface or by adding approved admin operations in OpenAPI before guardian self-service is considered complete.
...
- **SC-003a**: School-admin provisioning tests confirm guardian-user links can be created and deactivated only by permitted same-school administrators and that deactivated links immediately block guardian self-service access.
