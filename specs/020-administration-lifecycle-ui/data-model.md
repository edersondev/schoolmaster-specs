...
embedded permissions and requires only `roles.view`. The role detail view and
that dialog reuse one responsive permission grid that presents `code` as
Permission name and `name` as Description, with one column by default, two at
the small breakpoint, and four on large screens.
