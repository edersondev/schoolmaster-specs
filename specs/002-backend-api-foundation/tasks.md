...
- [X] T005 Create product API route entry point in `routes/api.php`
- [X] T006 Update Laravel routing registration to include API routes in `bootstrap/app.php`
- [X] T007 Remove the default product-facing welcome route from `routes/web.php`
- [X] T008 Remove the default product-facing Blade view from `resources/views/welcome.blade.php`
...
- [X] T012 [P] Create API response envelope helper in `app/Http/Resources/ApiResponse.php`
- [X] T013 [P] Create API exception response mapping for validation, unauthorized, forbidden, tenant mismatch, lockout, token rejection, and not found outcomes in `bootstrap/app.php`
- [X] T014 [P] Create tenant context value object in `app/DTOs/TenantContext.php`
- [X] T015 [P] Create tenant context resolver service in `app/Services/TenantContextResolver.php`
- [X] T016 [P] Create audit event service interface and base service in `app/Services/AuditEventService.php`
- [X] T017 [P] Create base authorization policy scaffolding for platform and school scopes in `app/Policies/ScopePolicy.php`
...
- [X] T019 [P] [US1] Add feature test for API route registration and no product welcome route in `tests/Feature/BackendReadinessTest.php`
- [X] T020 [P] [US1] Add feature test for `/specs` source files availability in `tests/Feature/SpecsSubmoduleReadinessTest.php`
- [X] T021 [P] [US1] Add unit test for route prefix expectations in `tests/Unit/ApiRoutePrefixTest.php`

### Implementation for User Story 1

- [X] T022 [US1] Wire API route group placeholder under `/api/v1` in `routes/api.php`
...
- [X] T027 [P] [US2] Add unit tests for API success and error envelope shapes in `tests/Unit/ApiResponseTest.php`
- [X] T028 [P] [US2] Add unit tests for tenant context resolution states in `tests/Unit/TenantContextResolverTest.php`
- [X] T029 [P] [US2] Add unit tests for login attempt lockout threshold in `tests/Unit/LoginAttemptControlTest.php`
- [X] T030 [P] [US2] Add unit tests for audit event payload sanitization in `tests/Unit/AuditEventServiceTest.php`
- [X] T031 [P] [US2] Add feature tests for token rejection response envelopes in `tests/Feature/AuthTokenRejectionTest.php`

### Implementation for User Story 2

- [X] T032 [P] [US2] Create login attempt control service for email/IP counting and 15-minute lockout windows in `app/Services/LoginAttemptControlService.php`
- [X] T033 [P] [US2] Create authentication token lifecycle service for 8-hour expiry and logout revocation in `app/Services/AuthTokenLifecycleService.php`
- [X] T034 [P] [US2] Create authentication audit event types in `app/DTOs/AuditEventData.php`
- [X] T035 [US2] Implement sanitized audit recording for login success, login failure, logout, token rejection, and school lifecycle events in `app/Services/AuditEventService.php`
- [X] T036 [US2] Implement tenant context resolution for session-bound and `X-School-Id` contexts in `app/Services/TenantContextResolver.php`
- [X] T037 [US2] Implement API response envelope methods for success, paginated, validation, unauthorized, forbidden, tenant mismatch, lockout, token rejection, and not found responses in `app/Http/Resources/ApiResponse.php`
- [X] T038 [US2] Register shared exception rendering for validation, authorization, tenant mismatch, lockout, token rejection, and not found outcomes in `bootstrap/app.php`
- [X] T039 [US2] Add Form Request base behavior for rejecting undocumented fields in `app/Http/Requests/ApiFormRequest.php`
...
- [X] T040 [P] [US3] Add authentication feature tests for login success, failed login, lockout, logout revocation, 8-hour token expiry, inactive user, and inactive school in `tests/Feature/AuthApiTest.php`
- [X] T041 [P] [US3] Add current-user feature tests for resolved school, roles, permissions, unauthorized token, and tenant mismatch cases in `tests/Feature/CurrentUserApiTest.php`
- [X] T042 [P] [US3] Add school management feature tests for list, create, get, update, activate, deactivate, validation, forbidden, and not found cases in `tests/Feature/SchoolManagementApiTest.php`
- [X] T043 [P] [US3] Add audit feature tests for auth and school lifecycle events in `tests/Feature/AuditEventApiBehaviorTest.php`

### Implementation for User Story 3

- [X] T044 [P] [US3] Create School model with UUID, status, and factory support in `app/Models/School.php`
- [X] T045 [P] [US3] Update User model for UUID, status, school relationship, and role relationship expectations in `app/Models/User.php`
- [X] T046 [P] [US3] Create Role model and Permission model with scope/status relationships in `app/Models/Role.php` and `app/Models/Permission.php`
- [X] T047 [P] [US3] Create authentication and school foundation migration in `database/migrations/2026_05_14_000001_create_schoolmaster_foundation_tables.php`
- [X] T048 [P] [US3] Create AuditEvent model and migration for append-only auth/school audit records in `app/Models/AuditEvent.php` and `database/migrations/2026_05_14_000002_create_audit_events_table.php`
- [X] T049 [US3] Create login request validation for email, password, optional school_id, and undocumented-field rejection in `app/Http/Requests/Auth/LoginRequest.php`
- [X] T050 [US3] Create school create and update request validation in `app/Http/Requests/Schools/StoreSchoolRequest.php` and `app/Http/Requests/Schools/UpdateSchoolRequest.php`
...
- [X] T052 [US3] Implement authentication service for login, current user, logout revocation, token expiry checks, inactive checks, lockout, and audit events in `app/Services/AuthService.php`
- [X] T053 [US3] Implement school service for list, create, get, update, activate/deactivate, platform authorization checks, and audit events in `app/Services/SchoolService.php`
- [X] T054 [US3] Create AuthController for `login`, `me`, and logout-adjacent revocation behavior if documented by OpenAPI in `app/Http/Controllers/Api/V1/AuthController.php`
- [X] T055 [US3] Create SchoolController for list, create, get, and update operations in `app/Http/Controllers/Api/V1/SchoolController.php`
...
- [X] T060 [P] Review all changed backend files for PSR-12 compliance and run formatter if configured in `composer.json`
