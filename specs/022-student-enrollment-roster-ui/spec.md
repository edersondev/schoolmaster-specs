...
- **FR-013**: The UI MUST allow authorized school administrators to assign eligible same-school teachers to class sections or rosters, display assignment status and academic-period context, source teacher candidates only from approved same-school active user data or a known teacher user ID, and show validation or conflict feedback for duplicate, inactive, cross-school, or incompatible assignments.
...
- Teacher assignment eligibility depends on an active same-school teacher-compatible user state covering the assignment effective start date; backend assignment creation remains authoritative for role compatibility, and the frontend must not add undocumented teacher lookup endpoints, role filters, or search parameters.
