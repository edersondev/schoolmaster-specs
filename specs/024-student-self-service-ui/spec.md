...
- **FR-005**: If no current active academic period is available from approved context, the UI MUST show a no-current-period state for assigned learning sets and any period-scoped academic summary, and MUST NOT send the required learning-set timeline request without an academic period.
...
- **FR-011**: The UI MUST list grade records using only the approved student grades route, documented pagination, current active academic-period filter where available, returned student-visible grade value, label, status, and recorded timestamp fields; manual period switching is outside this slice.
- **FR-012**: The UI MUST list attendance records using only the approved student attendance route, documented pagination, current active academic-period filter where available, returned student-visible attendance date, current attendance status, and lifecycle status fields; manual period switching is outside this slice.
...
- **AcademicPeriodContext**: Current active academic period used to scope assigned learning sets and period-specific academic summaries; manual period switching is outside this slice.
...
- Student learning-set listing requires an approved academic-period context; if no current period exists, the student timeline and period-scoped summary remain blocked until an approved context exists.
- Grade and attendance views use the current active academic period where available and do not expose manual period switching in this slice.
