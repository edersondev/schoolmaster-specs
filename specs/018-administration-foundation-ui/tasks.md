...
- [X] T054 [P] [US2] Implement role filters, table without inline permissions, read-only List permissions dialog, and school-permission create form with no editable scope control in `src/components/admin-system/roles/RoleFilters.vue`, `src/components/admin-system/roles/RoleTable.vue`, `src/components/admin-system/roles/RolePermissionsDialog.vue`, and `src/components/admin-system/roles/RoleForm.vue`
...
- [X] T057 [US2] Compose role list and school-only dedicated create routes with complete permission loading, fixed school scope, tenant gating, errors, and unsaved guard in `src/pages/admin-system/roles/RolesListPage.vue` and `src/pages/admin-system/roles/CreateRolePage.vue`
